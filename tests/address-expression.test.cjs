...
test("invalid expressions and PIDs never reach the API", () => {
  ...
  for (const pid of [0, -1, "", 2147483648, 1.5])
    assert.throws(() => expressions.resolve(pid, "foo.so", api), /PID/);
  assert.equal(calls, 0);
});

test("module failures, malformed responses, unsigned overflow and cancellation propagate", async () => {
  await assert.rejects(
    expressions.resolve(42, "missing.so", async () => {
      throw new Error("module not found in pid 42 maps");
    }),
    /module not found/,
  );
  for (const body of [
  ...
  await assert.rejects(
    expressions.resolve(42, "foo.so-1", async () => ({ address: "0" })),
    /64-bit/,
  );
  await assert.rejects(
    expressions.resolve(42, "foo.so+1", async () => ({
  ...
  controller.abort();
  await assert.rejects(
    expressions.resolve(42, "foo.so", async () => ({ address: "0" }), {
