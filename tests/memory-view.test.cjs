...
test("module-relative address expressions support arithmetic using the backend base", async () => {
  ...
  assert.equal(
    await expressions.resolve(1, "libgame.so + 0x1234 - 0x34", api),
    "0x0000000000002200",
  );
  assert.equal(
    await expressions.resolve(1, "/data/app/libgame.so+20", api),
    "0x0000000000001020",
  );
  assert.equal(
    expressions.resolve(1, "0x10 + 0x10", api),
    "0x0000000000000020",
  );
  await assert.rejects(
    expressions.resolve(1, "missing.so + 1", async () => {
      throw new Error("Module not found");
    }),
    /Module not found/,
  );
  await assert.rejects(
    expressions.resolve(1, "libgame.so - 0x2000", api),
    /exceeds/,
  );
});
...
test("64-bit address validation and size limits", () => {
  assert.equal(
    view.request("123", "0X20000000000001", "256").addr,
    "0x0020000000000001",
  );
  assert.equal(view.request(123, "ff", 1).addr, "0x00000000000000ff");
  for (const addr of ["", "-1", "xyz", "10000000000000000", "0x"])
    assert.throws(() => view.address(addr));
  for (const pid of [0, -1, 2147483648, 1.5, ""])
    assert.throws(() => view.request(pid, "0", 1));
  for (const size of [0, 4097, -1, 1.5])
    assert.throws(() => view.request(1, "0", size));
...
test("memory form resolves a module expression before requesting bytes", async () => {
  ...
  const { controller, el } = harness(
    ...
    async () => "0x0000000000001020",
  );
  el("Pid").value = "42";
  el("Addr").value = "libgame.so + 0x20";
  el("Size").value = "2";
  await el("Form").fire("submit");
...
test("memory module resolution uses the PID and shares cancellation with memory reads", async () => {
  ...
  const { controller, el } = harness(api, (pid, addr, options) =>
    expressions.resolve(pid, addr, api, options),
  );
  el("Pid").value = "42";
  el("Addr").value = "libgame.so+20-10";
  el("Size").value = "2";
  ...
  pending[1].resolve({ address: "20000000000001" });
  await second;
  pending[0].resolve({ address: "1000" });
  await first;
...
test("driver errors clear stale data and re-enable Read", async () => {
  const { controller, el } = harness(async () => {
    throw new Error("ioctl 601 failed: Bad address");
  });
...
function changedSpans(el) {
  return el("Rows")
    .children.flatMap((row) => row.children.flatMap((cell) => cell.children))
...
test("refresh highlights only changed bytes in hex and ASCII for 500ms", async (t) => {
  ...
  assert.equal(
    el("Rows").children[0].children[2].textContent,
    "<" + "A".repeat(15),
  );
  ...
  const { controller, el } = harness((path) => {
    ...
    if (calls.length > 1)
      return new Promise((resolve) => {
        finish = () => resolve(response(path, [66, 67]));
      });
    return Promise.resolve(response(path, [65, 66]));
  });
...
test("auto refresh pauses for inactive tabs and hidden documents", async (t) => {
  t.mock.timers.enable({ apis: ["setTimeout"] });
  let calls = 0;
  const { controller, el, doc } = harness(async (path) => {
    calls++;
  ...
  const { controller, el } = harness((path, options) => {
    ...
    if (calls === 2)
      return new Promise((resolve) => {
        ...
        finish = () => resolve(response(path, [1, 2]));
      });
...
test("polling errors stop auto refresh and do not retain stale data", async (t) => {
  ...
  assert.match(
    ...
    /ioctl 601 failed.*Auto refresh stopped/,
  );
  assert.equal(el("Rows").children.length, 0);
  assert.equal(el("Read").disabled, false);
  t.mock.timers.tick(3000);
...
test("HTML includes all view controls and loads helper before app", () => {
  ...
  assert(
    html.indexOf('src="./memory-view.js"') < html.indexOf('src="./app.js"'),
  );
});
