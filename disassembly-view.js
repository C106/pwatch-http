(function (root) {
  ...
  function operandTarget(row, arch) {
    ...
    const direct =
      arch === "arm64"
        ? /^(b|bl|b\.[a-z]+|cbz|cbnz|tbz|tbnz|adr|adrp|ldr|ldrsw|prfm)$/.test(
            mnemonic,
          )
    ...
    if (direct) {
      ...
      if (match) {
        ...
        if (value < END_ADDRESS)
          return {
            start: text.length - match[1].length,
            end: text.length,
            addr: memory.hex(value),
            indirect: false,
          };
      }
    }
    ...
    const match =
      /\[(?:(rip|eip)(?:\s*([+-])\s*(0x[0-9a-f]+|[0-9]+))?|(0x[0-9a-f]+|[0-9]+))\]/i.exec(
        text,
      );
    ...
    if (match[1]) {
      const displacement =
        BigInt(match[3] || "0") * (match[2] === "-" ? -1n : 1n);
      value = BigInt.asUintN(
        match[1].toLowerCase() === "eip" ? 32 : 64,
        row.addr + BigInt(row.bytes.length) + displacement,
      );
    } else {
  ...
  function request(input) {
    ...
    if (query.size > 4082)
      throw new Error("Page size must be between 1 and 4082 bytes.");
    if (!["arm64", "x86_64"].includes(input.arch))
      throw new Error("Select ARM64 or x86-64.");
    const start = memory.address(query.addr);
    if (input.arch === "arm64" && start % 4n)
      throw new Error("ARM64 addresses must be 4-byte aligned.");
  ...
  function nextPage(query, consumed) {
    ...
    return {
      ...query,
      addr: memory.hex(next),
      size: Number(
        end - next < BigInt(query.size) ? end - next : BigInt(query.size),
      ),
    };
  }
  ...
  function create(
    doc,
    api,
    loadDecoder = () => root.Disassembler.load(),
    actions = {},
    resolveAddress = (_pid, expression) => expression,
  ) {
    ...
    function rememberScroll() {
      if (loaded && visits[visitIndex])
        visits[visitIndex].scroll = el.View.scrollTop || 0;
    }
    function renderHistory() {
      ...
      entries.forEach((entry, index) => {
        ...
        option.textContent = entry
          ? `${index + 1}. PID ${entry.query.pid} | ${entry.query.arch} | ${entry.query.addr}`
          : "No jump history";
    ...
    function render(result, query) {
      ...
      for (const row of result.rows) {
        ...
        if (target) {
          ...
          link.addEventListener("click", () => {
            ...
            return read({
              ...
              size: Number(
                remaining < BigInt(query.size) ? remaining : BigInt(query.size),
              ),
            });
          });
        ...
        button.addEventListener("click", () =>
          actions.breakpoint?.(query.pid, memory.hex(row.addr)),
        );
        td.append(button);
        tr.append(td);
        fragment.append(tr);
    ...
    async function read(input, navigation = "jump", destination = -1) {
      ...
      try {
        ...
        let resolved = resolveAddress(input.pid, input.addr, {
        ...
        const prefix =
          expression !== query.addr
            ? `Resolved ${expression} to ${query.addr}. `
            : "";
        ...
        const body = await api(
          `/memory/read?${new URLSearchParams(wireQuery)}`,
        ...
        if (restoring) {
          ...
          if (
            !last ||
            ["pid", "addr", "arch", "size", "end"].some(
              (key) => last[key] !== query[key],
            )
          ) {
        ...
        visits[visitIndex] = {
          query,
          pages: [...history],
          scroll: el.View.scrollTop,
        };
        const invalid = result.rows.filter((row) => row.invalid).length;
        message(
          `${prefix}PID ${query.pid} | ${query.arch === "arm64" ? "ARM64" : "x86-64"} | ${query.addr} | ${result.rows.length - invalid} instructions | ${result.consumed} bytes${invalid ? ` | ${invalid} invalid/incomplete` : ""}`,
        );
      } catch (error) {
        if (current === version)
          message(
            error.name === "AbortError"
              ? "Disassembly request timed out. Retry Read / Jump."
              : error.message,
            true,
          );
      } finally {
    ...
    for (const input of [el.Addr, el.Size])
      input.addEventListener("input", editAddress);
    el.Pid.addEventListener("input", reset);
    el.Arch.addEventListener("change", reset);
    el.Refresh.addEventListener(
      "click",
      () => loaded && read(loaded, "refresh"),
    );
    el.Next.addEventListener("click", () => next && read(next, "next"));
    el.Prev.addEventListener(
      "click",
      () => history.length && read(history[history.length - 1], "previous"),
    );
    function visit(index) {
      if (!controller && visits[index])
        return read(visits[index].query, "visit", index);
    }
    el.Back.addEventListener("click", () =>
      visit(loaded ? visitIndex - 1 : visitIndex),
    );
    el.Forward.addEventListener("click", () => visit(visitIndex + 1));
    el.History.addEventListener("change", () =>
      visit(Number(el.History.value)),
    );
    reset();
    return {
      reset,
      setPid(pid) {
        reset();
        el.Pid.value = String(pid);
      },
      usePidIfEmpty(pid) {
        if (!el.Pid.value && pid) el.Pid.value = String(pid);
      },
