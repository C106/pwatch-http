...
(function (root) {
  ...
  const SUFFIX = /^(.+?)(\s*[+-]\s*(?:0x)?[0-9a-f]+(?:\s*[+-]\s*(?:0x)?[0-9a-f]+)*)$/i;

  function numeric(value) {
    const text = String(value).trim();
    if (!NUMBER.test(text)) throw new Error(`Invalid hexadecimal address: ${value}`);
    const digits = text.replace(/^0x/i, "");
    const result = BigInt(`0x${digits}`);
    if (digits.length > 16 || result > MAX) throw new Error("Address exceeds the 64-bit range.");
    return result;
  }
  ...
  function parse(expression) {
    ...
    let base = text, tail = "", quoted = false;
    if (text[0] === '"' || text[0] === "'") {
      ...
      if (match) { base = match[1].trim(); tail = match[2]; }
    }
    const terms = [];
    while (tail.trim()) {
      const term = tail.match(/^\s*([+-])\s*((?:0x)?[0-9a-f]+)(?=\s*[+-]|\s*$)/i);
      if (!term) throw new Error("Offsets must be hexadecimal numbers joined by + or -.");
      terms.push({ sign: term[1] === "-" ? -1 : 1, value: numeric(term[2]) });
      tail = tail.slice(term[0].length);
    }
    if (!quoted && NUMBER.test(base)) return { kind: "numeric", value: numeric(base), terms };
    if (!base || /[+"'\x00-\x1f]/.test(base) || (!quoted && (/^[+-]|^0x/i.test(base) || /\s|[()*]/.test(base))))
      throw new Error("Invalid address expression. Quote module paths containing spaces or operators.");
    return { kind: "module", module: base, terms };
  }

  function hex(value) {
    if (value < 0n || value > MAX) throw new Error("Resolved address exceeds the 64-bit range.");
  ...
  function resolve(pid, expression, api, options = {}) {
    ...
    return api(`/processes/${pid}/resolve?${params}`, { ...options, cache: "no-store" }).then(body => {
      options.signal?.throwIfAborted();
      if (!body || typeof body.address !== "string" || !NUMBER.test(body.address))
        throw new Error("Invalid module resolution response: expected a hexadecimal address string.");
