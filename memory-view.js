...
(function (root) {
  ...
  function address(text) {
    ...
    if (!/^[0-9a-f]{1,16}$/i.test(digits))
      throw new Error(
        "Address must be 1–16 hexadecimal digits (optional 0x prefix).",
      );
  ...
  function request(pid, addr, size) {
    ...
    if (!Number.isInteger(size) || size < 1 || size > 4096)
      throw new Error("Read size must be between 1 and 4096 bytes.");
    addr = address(addr);
    if (addr + BigInt(size - 1) > MAX_ADDRESS)
      throw new Error("Read extends beyond the 64-bit address range.");
  ...
  function validateResponse(body, query) {
    if (
      ...
      address(body.addr) !== address(query.addr) ||
      body.size !== query.size ||
      !Array.isArray(body.data) ||
      body.data.length !== query.size ||
      !body.data.every((b) => Number.isInteger(b) && b >= 0 && b <= 255)
    ) {
      throw new Error(
        "Invalid memory response: expected the requested address and byte count.",
      );
    }
  ...
  function page(query, direction) {
    ...
    if (direction < 0) {
      ...
      return {
        ...query,
        addr: hex(addr > BigInt(query.size) ? addr - BigInt(query.size) : 0n),
      };
    }
    ...
    return {
      ...
      addr: hex(next),
      size: Number(
        BigInt(query.size) < MAX_ADDRESS - next + 1n
          ? BigInt(query.size)
          : MAX_ADDRESS - next + 1n,
      ),
    };
  ...
  function create(doc, api, resolveAddress = (_pid, expression) => expression) {
    ...
    function render(formatted, data, previous) {
      ...
      formatted.forEach((row, rowIndex) => {
        ...
        for (let i = 0; i < length; i++) {
          ...
          for (const [cell, text] of [
            [bytes, value.toString(16).padStart(2, "0")],
            [
              ascii,
      ...
      const top = el.View.scrollTop,
        left = el.View.scrollLeft;
    ...
    async function read(input, mode = "jump") {
      ...
      try {
        ...
        const body = await api(`/memory/read?${new URLSearchParams(query)}`, {
          signal: activeController.signal,
          cache: "no-store",
        });
        if (current !== version) return;
        ...
        const previous =
          loaded &&
          ["pid", "addr", "size"].every((key) => loaded[key] === query[key])
            ? snapshot
            : null;
        ...
        loaded = query;
        message(
          `${prefix}PID ${query.pid} · ${query.addr} – ${hex(address(query.addr) + BigInt(query.size - 1))} · ${query.size} bytes${previous ? ` · ${changed} changed` : ""}`,
        );
      } catch (error) {
        if (current === version) {
          loaded = null;
          ...
          message(
            (error.name === "AbortError"
              ? "Memory request timed out. Retry Read / Jump."
              : error.message) + (stopped ? " Auto refresh stopped." : ""),
            true,
          );
        }
      } finally {
    ...
    for (const input of [el.Pid, el.Addr, el.Size])
      input.addEventListener("input", reset);
    el.Refresh.addEventListener(
      "click",
      () => !controller && loaded && read(loaded, "refresh"),
    );
